...
### VPN/AnyConnect: пользователи не проходят 2FA
- Проверить доступность UDP 1812 от ASA/FTD.
- Проверить `RADIUS_SECRET` и конфиг клиента.
- Проверить метрики `radius_requests_total` и логи RADIUS.
- Проверить Redis/БД (`redis_ping_total`, `db_ping_total`).
- Проверить каналы доставки (SMS/Push/Call).

### VPN/AnyConnect: высокий процент reject/timeout
- Проверить rate limiting и блокировки.
- Проверить доступность внешних провайдеров.
- Временно увеличить таймауты или отключить нестабильный канал.
