...
func TestServiceCreateInvite(t *testing.T) {
	...
	resp, err := svc.CreateInvite(context.Background(), dto.AdminInviteCreateRequest{
	...
	if invites.created == nil || invites.created.Email != "alice@example.com" || invites.created.Status != models.InvitePending {
		t.Fatalf("unexpected invite: %+v", invites.created)
	}
}
...
func (f fakeRadiusClientRepo) SetEnabled(ctx context.Context, id string, enabled bool) error {
	return nil
}
...
func (f fakeRadiusReqRepo) UpdateResult(ctx context.Context, id string, result models.RadiusResult) error {
	return nil
}
...
func (r *recordSessionRepo) RotateRefreshHash(ctx context.Context, id string, newHash string) error {
	return nil
}
